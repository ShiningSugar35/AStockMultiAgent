...
2. Reuse valid existing snapshots first. Then follow deterministic local/API provider fallback. A failure from the preferred provider is not a terminal state when a configured backup can answer the same question.
3. If deterministic sources remain insufficient, automatically continue with Web search before asking the user. Preferred order: exchange / CNINFO / issuer IR / regulator official sources, then reputable professional/public sources for corroboration. Prefer sources that do not require account creation, API keys, cookies, or user interaction.
