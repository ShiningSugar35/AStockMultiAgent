...
1. For normal research, start from the current acquisition/FinancialIntegrity lineage and run `uv run astock context-plan`; use `uv run astock probe` only for explicit capability diagnostics. Confirm source versions, units, periods, and point-in-time availability.
