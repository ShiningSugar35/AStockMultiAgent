...
1. Start from `uv run astock local-portfolio-sync-paper` when the paper account exists, then read `uv run astock local-portfolio-status` and `uv run astock continuous-monitor-status`. The Git-ignored Markdown files are the Agent-facing state; the paper ledger is the deterministic order/fill source, while `$continuous-investment-monitor` is the durable incremental-change source.
2. On **every investment-related Agent task**, review current holdings in addition to the user's main question. Consume material unresolved monitor events/pending tasks for held names first, then do delta research rather than repeating the full company report: check new official disclosures, material financial changes, catalysts, thesis invalidation, valuation movement, and relevant market/execution changes since the last review.
3. The daemon may already have replayed confirmed open paper orders. Refresh hourly data/replay only when status shows a gap, stale boundary, source degradation, or path ambiguity; use 5m only when the hourly path is materially ambiguous.
