...
2. Resolve the active internal research archetype with `uv run astock industry-research-resolve QUERY`; treat it as a research method, not a certified official industry label. Then load the public-core methodology bundle with `uv run astock industry-research-methodology QUERY`. The bundle supplies source-audited questions, KPIs, valuation lenses and red flags from professional/regulatory/industry-standard methodology sources; it is methodology only and cannot certify any current company fact. If the sector-specific bundle cannot be resolved, continue with its `ARCHETYPE_FALLBACK` + `GENERIC_COMPETITIVE_CORE` for a known-but-not-yet-mapped archetype, or `GENERIC_FALLBACK` for a fully unclassified query and record the degradation internally—do **not** return `NEEDS_INFO` solely because a methodology pack or private/blogger Skill is absent. Record official taxonomy separately when available.
3. Map the value chain: upstream inputs, production/service bottlenecks, channels, customers, substitutes, regulatory constraints and profit-pool distribution. Apply the resolved public-core methodology as a question/metric checklist, but source every company/industry conclusion from current admissible Evidence. Connect every material link to an observable driver or mark it unresolved.
