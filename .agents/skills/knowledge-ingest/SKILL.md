...
12. After an audited active registry is published, historical Semantic/Distillation/Reviewed pipeline rows may leave the hot SQLite only through `knowledge-cold-archive-run --confirm`. Preserve every parent row still referenced by a surviving hot table, require zstd Parquet + ObjectStore manifest + archive audit + successful full restore proof, and never include raw SourceSnapshot/Evidence/Zhihu version/ObjectStore source bodies in that deletion scope. Tiny immutable knowledge metadata Parquet may be compacted by existing partition; additive historical schema drift is represented with nullable missing fields, not rewritten facts.
