...
1. This is **not** a real-money trading bot. The default operating model combines the deterministic Continuous Monitor with session recovery: when enabled, the monitor may refresh/replay already-confirmed open paper orders continuously; when an Agent investment task starts, inspect the paper account plus `continuous-monitor-status`, reconcile any remaining gap, and mirror the result into Git-ignored `user_state/*.md`.
2. Run `uv run astock paper-status` and reject mutation if the ledger is unbalanced or otherwise corrupt. The SQLite ledger remains the deterministic source for cash, orders, fills and settlement; `user_state/portfolio.md`, `orders.md`, and `trades.md` are the human/Agent-facing local mirror. The monitor may call the same replay service but never creates a new order or direct position mutation.
3. Default replay resolution is **60m**. If the monitor has not already advanced the checkpoint, run `uv run astock sync-hourly <symbol> --market <market>` and `uv run astock paper-replay <symbol> --cursor <time> --market <market>`. Hourly OHLC may establish that a limit price was touched, but cannot prove intrabar ordering or queue priority; preserve the `PROVIDER_1H_APPROX` qualification.
