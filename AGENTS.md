...
- **荐股、投资组合推荐、买入/卖出判断、持仓处置、个股/公司完整分析属于 material investment request，没有短答捷径。** 必须在当前用户请求内自动跑完 intent 对应的 canonical capability graph 与全部适用 Repo Skills：全市场/当前环境 → bounded 候选 → 单股深研 → 财务完整性/治理/行业/事件 → 预测估值 → 独立 Bull/Bear/Reviewer/Red Team → Committee/交易分类 → Portfolio/仓位与入场/退出/失效条件 → FullResearchInputReadiness → Mandatory Research DAG → RecommendationResearchReceipt → Publication Gate。`ResearchSeed`、Candidate、外部资料待补、team nonterminal、未完成 Committee/Portfolio、仅上游 READY 都只是内部中间态，严禁据此直接给 BUY/组合结论，也严禁以“下一步可以继续使用 AStockMultiAgent……”结束本轮。
...
- **material investment request 必须先解析投资预期。** 本金与目标年化分别按“本轮用户明确值 → 最近已注册请求/持仓研究/`RecommendationResearchReceipt` 中可验证的用户已记录值 → 默认本金 100000 元 / 目标年化 100%”解析，缺哪一项只补哪一项；模型默认值不能覆盖用户历史值，不能从账户市值或助手示例猜本金。目标年化是组合规划目标，不是收益承诺；它可以收紧可接受买入价、形成目标利润/下行情景/目标缺口与加减仓复核条件，但不得放宽 Committee、Publication Gate、集中度、流动性、风险预算、保证金/做空或真实券商禁令。目标与当前赔率不相容时保留现金并呈现缺口，不得为追目标强行加杠杆或凑仓。**隐私规则从宽且基于来源/用户意图：荐股规划本金或预算、目标年化/目标盈利、推荐配置金额和建议股数、证券公开价格、发行人财务与股东公开数据均可正常展示；只有真实/模拟账户或现有持仓的个性化事实（如持仓本金/成本、账户资产/现金、当前持仓股数、实际持仓盈亏）默认不主动输出。若用户在本轮明确提及、询问或要求查看对应账户/持仓数值，则可从已核实的 canonical 私有记录中回答，不得因“隐私”拒绝用户主动查询自己的数据。**
...
- 新当前公司研究默认要求 Phase 9 机构级基本面层：`EvidenceSufficiencyReport → IndustryProfile / CompanyEconomicsProfile → DriverTree → ForecastPack → ValuationPack → FundamentalModelBundle → InstitutionalDecisionContext`。Forecast/Valuation 数值由 Python 确定性复算；Serenity Growth/Valuation 不得维护第二套平行数值账本。CURRENT 的 expected return / market-implied expectations 只能使用绑定注册 artifact/hash、来源可追溯且在本轮 acquisition/recovery 完成后冻结进当前决策快照的 `MarketPriceAnchor`；不得用用户提问时刻做 anti-lookahead cutoff。Bundle/DecisionContext 作为 Committee 共享 PRIMARY 冻结输入，不新增投票席位或权重；显式 historical/replay 才保留独立 PIT/历史可见性规则，历史 Phase 6 recorded 链只作兼容。
- 当前全市场发现保留纯 market-score blind tranche，并用独立 `BREADTH_CHALLENGER` 研究预算补充 blind 未覆盖、仍接近 cutoff 的 broad economic domain；Existing Candidate、Knowledge/Serenity Skill 只能复用/增强研究，不能改写 blind membership 或 breadth fairness。当前推荐另使用 deterministic `EntryQualitySnapshot` 区分低位筑底、趋势确认、falling knife 与 extended，只能缩放已通过 Full Research/Committee 的初始仓位，不能产生 BUY 权威。关键人事/工商/信用/司法/合作方事件通过 typed Enterprise Intelligence + Evidence/Snapshot lineage 解析；`enterprise.*` 的可选商业能力按 MCP→API→权威 Web 回退，但交易所/CNINFO/政府/法院正式来源始终优先，冲突不得 last-write-wins。
...
- Agent 只能提交 `PROPOSED` proposal；Python deterministic validator 决定是否形成 `VALIDATED` artifact。CURRENT Planner 必须自动补回 active policy 的 core acquisition、Evidence/来源 lineage、Financial Integrity/Fundamental 等 mandatory gate；Agent 不得删除公司质量或最终发布硬门。显式 historical/replay 的 PIT/anti-lookahead gate 只属于对应历史模式，不得作为 CURRENT 请求的提问时刻截断器。
...
- Deterministic Core 继续永久强制：CURRENT 的来源认证/对象完整性/证券身份/事实冲突、会计与数学恒等式、最终当前决策快照、不可变正式 receipt、账本平衡、模拟盘人工确认、`paper_ledger_write_allowed=false` 的研究链、`broker_execution_allowed=false`。AI 不得通过 prompt、proposal、Skill 或 fallback 绕过这些边界。损坏或漂移的中间对象优先触发重抓/重建而不是直接终止。PIT/未来函数防护仅对显式 historical/backtest/replay 模式强制；不得把用户提问时刻当 CURRENT 的未来数据边界。这里的边界同时包括公司质量、CriticalVeto、Committee 与 Publication Gate。
- 外部能力生产备用只认 canonical M-06 资格报告。2026-09-13 当前固定证据下，Arelle、Docling、Playwright MCP、AKShare、Crawl4AI、changedetection.io、QCC Enterprise Intelligence MCP/API、Tianyancha Enterprise Intelligence API、`source-qualification-auditor`、`report-visual-qa`、`schema-drift-recorder` **全部保持 `SHADOW`**（GDELT 仍为 discovery-only）：外部企业情报的公开产品存在不等于当前部署拥有凭据、数据权利、稳定 endpoint 或生产资格；`source-qualification-auditor` 也仍需真实 controlled-live 输入/输出与延迟证据。任何候选缺 rights/provenance/recorded/live/SBOM/安全/退出门时都不得为满足计划配额硬升生产级；若该能力服务显式 historical/replay，再额外要求相应 PIT/历史可见性证据。治理 Skill 也绝不能提升底层数据源的官方性或完整性。
...
- MCP 断连、新会话或上下文丢失后，恢复事实源顺序固定为：`AGENTS.md → docs/architecture/ 现行架构合同 → 开发计划.md → 进度验收.md → 唯一 durable run → Git/测试/运行工件`。禁止依赖聊天记忆、创建平行 long-run，或重新编写超长接管提示词替代仓库事实。
...
- Gemini 必须继承本仓库 CURRENT source provenance、Universe lineage、Evidence、对象完整性、账本、Provider、paper execution、人工确认和真实交易禁止等硬约束；显式 historical/replay 任务再额外继承 PIT/历史可见性约束。不得把辅助 Agent 的意见直接提升为正式研究、正式推荐、模拟成交或交易动作。
