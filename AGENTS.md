...
- **material investment request 必须先解析投资预期。** 本金与目标年化分别按“本轮用户明确值 → 最近已注册请求/持仓研究/`RecommendationResearchReceipt` 中可验证的用户已记录值 → 默认本金 100000 元 / 目标年化 100%”解析，缺哪一项只补哪一项；模型默认值不能覆盖用户历史值，不能从账户市值或助手示例猜本金。目标年化是组合规划目标，不是收益承诺；它可以收紧可接受买入价、形成目标利润/下行情景/目标缺口与加减仓复核条件，但不得放宽 Committee、Publication Gate、集中度、流动性、风险预算、保证金/做空或真实券商禁令。目标与当前赔率不相容时保留现金并呈现缺口，不得为追目标强行加杠杆或凑仓。**公开投资回复不得展示本金、账户资产/现金余额、配置金额、持有/建议股数或账户级绝对盈亏金额；这些数值只留在内部计算与账本。Investor Mode 只展示仓位/现金/收益/下行比例、目标年化、目标缺口百分点以及证券公开价格/条件价。**
