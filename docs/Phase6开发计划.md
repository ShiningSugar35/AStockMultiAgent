...
状态：开发前计划。Phase 4 已完成研究和持仓生命周期闭环；Phase 5 的确定性蒸馏内核已实现但线上覆盖及人工审核仍为 `PARTIAL`。本文件只定义 Phase 6 的实现边界，不把计划写成已经具备的能力。
...
只有当冻结输入、不可覆盖硬门槛、选择性反方、NEEDS_INFO 闭环、DecisionPack、每决定一份 TradeProtocol、预算降级、Codex 严格包装、恢复审计、真实 migration 和全仓验收全部由代码与测试证明后，Phase 6 才能标记完成。计划文件、Schema 存在或单个 happy-path 测试不能代替终验。
