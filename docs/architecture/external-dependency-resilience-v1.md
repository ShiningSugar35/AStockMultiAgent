...
20. v0.3.0 R-01 已把上交所/深交所官方证券 denominator freeze 接入既有 `instrument.master` 证明链；新的官方 freeze 只从真实 `available_to_system_at` 起生效，禁止回填历史。该 v0.3.0 基线中的国家统计局、人民银行、财政部、发改委 legacy provider authority 保持 recorded-first、`live_supported=false`；后续 WP-05 已新增独立 `OfficialMacroCaptureService` 处理具名 current official family 的 raw-first live capture，不修改 legacy Router 资格语义，也不把局部 family 成功提升为 exhaustive macro coverage。北交所财报/公司行动只准 exact-item official capture，不声称 exhaustive enumeration 或 negative proof。
