...
- 新字段缺省为 None 并从旧结构序列化中排除；旧 receipt 内容、语义 hash 和重放不回填新目标。公开报告正常显示荐股规划本金/预算、目标年化/目标盈利、推荐配置金额/建议股数、证券公开价格和发行人公开事实；只有真实/模拟账户或既有持仓的个性化本金/成本、账户资产/现金、当前持仓股数和实际盈亏默认不主动展示。用户本轮明确提及或询问对应账户/持仓信息时，可以从已核实 canonical 私有记录中展示该类别；仍禁止泄漏 registry、内部字段或用户未请求的其它私人类别。
