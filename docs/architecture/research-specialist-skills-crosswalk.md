...
| `reviewer` | `.agents/skills/investment-red-team/SKILL.md` | Independent reproduction; base rates; alternative explanations; pre-mortem and kill criteria | Typed `ChallengerAssessment` after separate frozen `bull-case` and `bear-case` outputs with distinct contexts | Critical fact not reproduced, independence not proven, or material downside outside the admitted range |
