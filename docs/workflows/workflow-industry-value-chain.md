...
5. Register `IndustryResearchOutcome` with `research-team-role-output`, then complete the task with `research-team-task-result`. `INDUSTRY_PROFILE=true` requires frozen evidence and non-circular peer methodology.
