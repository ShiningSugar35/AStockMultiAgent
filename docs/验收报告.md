...
# Phase 8 P8.0 默认禁用边界验收报告

验收日期：2026-07-17

验收结论：`PASS`（仅指 P8.0 只读安全边界）

真实 Phase 8 证据状态：`NOT_RUN`，不得表述为 `EVIDENCE_READY`

## 1. 自动化验收

| 检查 | 实测结果 |
|---|---|
| 全仓测试 | `232 passed, 8 skipped`，耗时 131.52 秒 |
| Ruff | `All checks passed` |
| Pyright | `0 errors, 0 warnings, 0 informations` |
| P8.0 专项单元测试 | 6 项通过：状态、缺口、哈希、准入、完整性阻断、零写入 |
| CLI/探针/Repo Skill | 命令存在、探针字段正确、未知 `--force-enable` 参数被拒绝 |

8 个跳过项仍是原有显式环境门控制的真实 OCR 基准和 live Provider probes，本轮没有把它们计为已运行通过。

## 2. 真实运行态验收

| 检查 | 实测结果 |
|---|---|
| Python | `3.12.10`，满足项目固定版本 |
| `adaptive-research-status` | `NOT_ENTERED_BY_DESIGN`；Phase 7 audit 为 `NOT_RUN` |
| 样本缺口 | 12 个月、100 个独立决定、5 个合格折、3 个合格市场状态 |
| 下一阶段 | 仅 `PHASE7_FORWARD_EVIDENCE_COLLECTION` |
| 四项安全能力 | adaptive weights、online learning、主模拟账本写入、券商执行全部为 false |
| SQLite | `integrity_check = ok`；`foreign_key_check = []` |
| Phase 7 表 | policy/study/arm/assignment/input/regime/observation/run/report/admission 调用前后均为 0 |
| Artifact 注册表 | 调用前后均为 494，没有新登记 P8.0 工件 |
| 主模拟账户 | 1 account、1 初始化 journal、0 order、0 fill、0 position |
| 资金与净值 | CASH 与 NAV 均为 100,000,000 分，冻结现金、费用和已实现盈亏均为 0 |

对 16 张关键表执行命令前后快照比较，结果为 `unchanged=true`。这证明 P8.0 是派生只读状态，没有通过旁路修改 Phase 7 或主模拟账户。

## 3. 安全边界验收

- 未新增 migration、运行工件、Parquet 事实、权重文件、模型依赖或 GPU 依赖。
- 未联网，未调用 Browser、MCP、外部 LLM 或券商接口。
- 未读取或提交私有 PDF、DOCX、知乎正文、Cookie、Chrome Profile、密钥或 `runtime/**`。
- admission 不合格时只转述冻结 gate/reason；对象、哈希、索引或审计异常时返回 `BLOCKED_BY_INTEGRITY` 并保持全部能力关闭。
- admission 即使未来变为 `ELIGIBLE_RULE_STATE_MACHINE_RESEARCH`，P8.0 也只返回 `AWAITING_EXPLICIT_RULE_RESEARCH_APPROVAL`，不会生成权重或启动研究。

## 4. 验收判定

P8.0 的 Schema、只读服务、CLI、probe、Repo Skill 和回归保护均已完成，可判定程序层通过。P8.1～P8.4 没有开发、没有授权，也没有被文档冒充为已实现。当前没有 P8.0 所需的注册、购买或手动浏览器操作；下一次可能需要人工确认的节点，只会出现在某个真实且审计通过的 admission 已满足全部硬门之后。

---

# 2026-07-17 恢复终验与 K5.5a 验收

验收日期：2026-07-17

代码结论：`PASS`

知识覆盖结论：`PARTIAL`，原因是知乎访问与评论覆盖尚未闭合，不是对象或索引损坏

Phase 8 结论：`NOT_ENTERED_BY_DESIGN`

## 1. 恢复点与完成性结论

- M3.2/M3.3、Phase 4、Phase 6、Phase 7 程序层和 Phase 8 P8.0 安全边界均已实际实现，本轮没有重复返工或把计划冒充代码。
- Phase 5 的本地摄入、结构解析、确定性蒸馏、私有草稿、候选 Skill、对象审计、SQLite/Parquet 对账均可用；三位线上作者的历史全集和评论范围仍未完成，所以 Phase 5 保持 `PARTIAL`。
- “寒武纪的鳄鱼”严格使用用户确认完整的 DOCX，不再发起线上采集；MR Dang、黄彦臻、派大星皮皮仍是仅有的线上采集对象。
- Phase 7 的真实 study、决定和 observation 仍为 0；Phase 8 缺 12 个月、100 个独立决定、5 个合格折和 3 个合格市场状态，动态权重、在线学习、模拟账本写入和券商执行继续全部关闭。

## 2. K5.5a 开放缺口边界去重

恢复实测对 MR Dang 回答首页再次执行低频 Python 探测，返回 `AUTH_REQUIRED`。系统正确新增一条失败证据，但旧聚合逻辑把同一页、同一游标的两次失败尝试算成两个内容缺口，导致报告从 7 膨胀为 8。

修正后：

- SQLite 仍保留 8 条开放失败历史，没有删除或覆盖审计证据。
- 面向用户的缺口按 `collection_scope + 业务游标` 去重；失败快照 ID 不再改变缺口身份。
- 列表页与评论页使用同一计数语义，不同页码、游标和评论范围仍分别计数。
- 损坏或非对象形式的游标 JSON 会返回显式存储错误，不会被忽略或误算为 0。

真实 `knowledge-coverage-audit` 结果：

| 来源 | 不同开放边界 | 未完成根评论范围 | 当前状态 |
|---|---:|---:|---|
| MR Dang | 2 | 0 | `PARTIAL` |
| 黄彦臻 | 2 | 1 | `PARTIAL` |
| 派大星皮皮 | 3 | 615 | `PARTIAL` |
| 寒武纪的鳄鱼 | 0 | 0 | `USER_CONFIRMED_COMPLETE_EXPORT` |
| 合计 | 7 | 616 | `PARTIAL` |

同一聚合审计还确认：缺失对象 0、Parquet 错配 0、待消费导入信封 0、长时间运行任务 0。

## 3. 两份私有蒸馏原料

| 原料 | 全量解析结果 | 结构化清洗结果 | 隐私状态 |
|---|---|---|---|
| 《价值投资功法》Mr.Dang.pdf | 249/249 页；244 页原生解析、5 页 OCR、0 空页、0 失败页；151,014 字符 | 171 个精确重复候选、809 个降权/移除候选、1,056 个方法段、28 个案例段；清洗可重建 | 原件哈希等于不可变原始对象哈希；`GIT EXCLUDED`；禁止对外再发布 |
| 寒武纪的鳄鱼 知乎文章.docx | 2,032/2,032 块；1,909 个非空块、123 个空块、0 不支持对象、0 解析缺口；93,323 字符 | 40 个精确重复候选、81 个降权/移除候选、421 个方法段、37 个案例段；清洗可重建 | 用户确认完整导出；原件哈希等于不可变原始对象哈希；`GIT EXCLUDED`；禁止对外再发布 |

两份清洗报告与方法覆盖报告均为 `PENDING` 人工审核；零命中类别没有被擅自写成 `AUTHOR_SILENT`。原始内容永久保留，清洗只改变候选权重，不删除源材料。

## 4. 蒸馏、草稿与引用链

| 作者 | 蒸馏单元 | 观点草稿 | 候选 Skill | 覆盖语义 |
|---|---:|---:|---:|---|
| MR Dang | 5,942 | 148 | 14 | 本地书籍完整；线上历史部分缺失 |
| 黄彦臻 | 1 | 0 | 0 | 仅现有想法；线上历史与评论部分缺失 |
| 派大星皮皮 | 615 | 125 | 14 | 已保存现有想法；后续历史与评论部分缺失 |
| 寒武纪的鳄鱼 | 1,856 | 128 | 12 | 用户确认完整 DOCX，不再线上抓取 |
| 合计 | 8,414 | 401 | 40 | 观点和 Skill 全部待人工处理 |

- 四位 `knowledge-distill-audit` 均为 `PASS`：规范化对象、源对象、Parquet 行、哈希和报告计数均无缺失或错配。
- 四位 `knowledge-draft-audit` 均为 `PASS`：私有载荷、源引用、候选引用、审批门禁和报告计数均无 finding。
- 401 条观点仍是 `SOURCE_EXCERPT_NOT_SYNTHESIZED/PENDING`，没有被包装成已验证投资结论；40 个候选 Skill 仍为 `NOT_RUN/PENDING`，没有进入生产路由或自动交易。

## 5. 浏览器与平台实测

- Python 优先采集在 MR Dang 回答范围返回 `AUTH_REQUIRED`，证明无凭据请求不能复用 Chrome 登录态。
- 已登录 Chrome 能显示 MR Dang 作者页；直接 API 导航被客户端拦截，页面接管、页面上下文请求和 CDP 结构化读取连续超时。
- 连续三条结构化读取路径失败后已停止重试并保留精确断点，避免重试风暴；未读取 Cookie、Profile、local storage 或明文会话信息，未绕验证码或访问控制。
- 未遇到需要注册、购买、短信或验证码的页面；当前阻断是 Chrome 控制/导出通道不稳定。

## 6. 自动化与运行态验收

| 检查 | 本轮结果 |
|---|---|
| M3.2/M3.3 定向测试 | 16 passed |
| Phase 4/6/7/8 定向测试 | 50 passed |
| K5.5a 定向测试 | 7 passed |
| 全仓测试 | 236 passed、8 skipped，0 failed，129.32 秒 |
| Ruff | All checks passed |
| Pyright | 0 errors、0 warnings、0 informations |
| `astock probe` | Python 3.12.10；SQLite integrity `ok`；Phase 7 `NOT_RUN`；Phase 8 `NOT_ENTERED_BY_DESIGN` |
| 主模拟账户 | 0 委托、0 成交、0 持仓；现金与 NAV 均为 100,000,000 分；账本平衡 |

8 个跳过项仍是受显式环境门控制的真实 OCR 基准和外部 live Provider probes，没有被计为已运行通过。第一次全仓测试因外层 120 秒限时被终止；放宽到 300 秒后从头重跑并得到上表正式结果，超时结果没有被冒充为通过。

## 7. Python、Git 与人工事项

- 当前解释器为 `D:\Environment\Python312\python.exe` / Python 3.12.10；Python 3.11/Launcher 卸载登记为 0，常见残余目录、`C:\Windows\py.exe` 和 `pyw.exe` 均不存在。
- 本轮删除了工作区内已被 Git 忽略的 `runtime/launcher-uninstall.log`，不再保留一次性卸载日志。
- Git 仍只追踪 `runtime/.gitkeep`；两份私有原件、运行数据库、对象正文、Cookie、Profile 和密钥均未被跟踪或暂存。
- 仍需人工处理的事项统一为：恢复稳定的登录态响应导出后补齐 7 个线上边界及 616 个评论范围；人工综合 401 条观点并评审 40 个候选 Skill；未来自然积累 Phase 7 的 12 个月/100 个独立决定证据。当前不需要注册或购买任何服务。
