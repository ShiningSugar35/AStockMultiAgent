...
class ResearchSkillService:
    ...
    def route(self, request: SpecialistRouteRequest) -> SpecialistRouteExecution:
        ...
        self._reject_explicit_selection_groups(explicit_manifests)
        self._reject_explicit_source_family_limits(explicit_manifests, registry)
        ...
        for manifest, match in eligible:
            ...
            if conflict is not None:
                ...
                continue
            selection_group_conflict = self._selection_group_conflict(
                manifest,
                selected_manifests,
            )
            if selection_group_conflict is not None:
                excluded[manifest.skill_id] = [
                    f"SELECTION_GROUP_ALREADY_FILLED:{selection_group_conflict}"
                ]
                continue
            family_limit = registry.source_family_limits.get(manifest.source_family)
            if (
                family_limit is not None
                and sum(item.source_family == manifest.source_family for item in selected_manifests)
                >= family_limit
            ):
                excluded[manifest.skill_id] = [
                    f"SOURCE_FAMILY_LIMIT:{manifest.source_family}:{family_limit}"
                ]
                continue
    ...
    @staticmethod
    def _selection_group_conflict(
        candidate: ResearchSkillManifest,
        selected: list[ResearchSkillManifest],
    ) -> str | None:
        if candidate.selection_group is None:
            return None
        for other in selected:
            if other.selection_group == candidate.selection_group:
                return other.skill_id
        return None
    ...
    def _reject_explicit_selection_groups(
        self,
        manifests: list[ResearchSkillManifest],
    ) -> None:
        for index, manifest in enumerate(manifests):
            conflict = self._selection_group_conflict(manifest, manifests[:index])
            if conflict is not None:
                raise ValueError("explicit specialist request contains duplicate selection groups")

    @staticmethod
    def _reject_explicit_source_family_limits(
        manifests: list[ResearchSkillManifest],
        registry: ResearchSkillRegistry,
    ) -> None:
        counts: dict[str, int] = {}
        for manifest in manifests:
            counts[manifest.source_family] = counts.get(manifest.source_family, 0) + 1
        for family, count in counts.items():
            limit = registry.source_family_limits.get(family)
            if limit is not None and count > limit:
                raise ValueError("explicit specialist request exceeds source family limit")
