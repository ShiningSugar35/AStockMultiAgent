...
影子研究只验证研究观点在形成后是否持续有效，不预测收益，也不产生自动交易。正式链固定为：

`ResearchMemo → ShadowDecision → Future Observation`

- `ResearchMemo`、委员会 `DecisionPack` 与 `TradeProtocol` 在信号时点冻结，决定必须精确引用 Memo
  对象哈希；正式事件由系统记录接收时间，超过登记时限的回填事件不得进入正式样本。
- 未来观察只读取信号后真实采集的未复权 5 分钟行情。源响应先写 ObjectStore，
  `SourceSnapshot` 保存采集和系统可得时间；观察再冻结标的、时间范围、源快照和逐条行情
  observation 的不可变 OHLCV 清单。入场/估值价格、成交量、持有期最高/最低价和 MFE/MAE
  必须与该清单复算一致。历史回放和回测只能进入探索研究，不能增加正式事件计数。
- 每个观察版本记录原观点、信号时间、价格、后续表现、观点状态和结构化失效原因；修订产生新版本，
  不覆盖旧对象。
- 独立性按公司、论点版本和事件身份确定，同一事件的多比较臂不重复计数。至少 100 个真实独立前向
  研究事件及所需未来观察形成前，Skill 表现、Committee 表现和 Research 质量统一报告
  `COLLECTING`。同一 `ResearchMemo` 或 `ShadowDecision` 在同一 study 中只能计为一个独立事件。
- 研究臂和权重在研究开始前冻结。Phase 7 禁止强化学习、动态或在线调权、自动修改 Skill，以及
  任何券商自动执行。
...
- Phase 7：冻结研究和权重的真实前向影子研究、未来观察与三维评估；不足 100 个独立事件时为
  `COLLECTING`。
