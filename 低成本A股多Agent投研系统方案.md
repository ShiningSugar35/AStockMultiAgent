...
> 文档职责固定为：总方案只写长期设计；未完成项以《开发计划》为准；《进度验收》只保留最近一次任务的有证据验收终态；影响当前理解的稳定实现事实由 `AGENTS.md`、现行架构/Workflow/Skill 文档承接。
> 当前代码进度、提交、测试和阶段状态，应以仓库中的 `开发计划.md`、`进度验收.md`、现行架构文档、PM 状态提交及 Git/Release 事实为准。
