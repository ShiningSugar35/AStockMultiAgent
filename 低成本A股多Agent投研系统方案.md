...
> 文档状态：HISTORICAL / FROZEN（2026-09-07 起不再追加当前计划、能力状态或验收结果）。
> 说明：本文件根据此前完整讨论、已确认的业务目标、数据边界和专家体系重建，只用于保存长期愿景与历史设计背景。
> 它描述“系统曾计划做什么、为什么这样设计、最终希望具备哪些能力”，**不能作为当前 GitHub 仓库的完成证明、开发计划或运行合同**。
> 当前入口以 `docs/README.md` 为准；未完成项看《开发计划》，最近验证看《进度验收》，机器事实看 Schema、配置、CLI、SQLite/Parquet/ObjectStore 与测试，重要决策看 `docs/adr/`。
> 本文与当前机器合同冲突时，以机器合同和 `AGENTS.md` 为准；需要恢复的稳定设计应迁入 current 架构/Workflow/Skill 或新增 ADR，而不是继续修改本文。
...
上述工程事实必须以 GitHub 仓库、PM 状态提交、`开发计划.md`、`进度验收.md`、现行架构文档和阶段/Release 事实为准。
